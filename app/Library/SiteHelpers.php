...
use App\Models\Lmssections;
use App\Models\Lmsmodules;
use App\Models\Lmsassign;
use App\Models\Lmssurveyassign;
use App\Models\Lmsdiscussionreplies;
use App\Models\Lmsunits;
use App\Models\Schoolassignregionaladmin;
use App\Models\Schoolassign;
use App\Models\School;
use App\Models\Survey;
use App\Models\Surveyassign;
use App\Models\Settings;
use App\Models\Lmsorders;
use App\Models\Answer;
use App\User;
use App\Models\Lmspermission;
			...
			if($is_allow ==1)
			{
				...
				if(count($menus2) > 0 )
				{
					...
					foreach ($menus2 as $row2)
					{
						...
						$customPermission = self::checkLMSPermission();

						if($row2->allow_guest == 1)
						{
							$is_allow = 1;
						} elseif(isset($customPermission[$row2->module]['view']) && $customPermission[$row2->module]['view']) {
							$is_allow = 1;
						}	else {
							$is_allow = (isset($p[Session::get('gid')]) && $p[Session::get('gid')] ? 1 : 0);
						}
...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	public static function showUploadedFile($file,$path , $width = 50, $extraFile=false) {
		...
		//	echo $files ;
			...
			if($extraFile && strlen($extraFile))
			return "<img src='".asset($extraFile)."' border='0' width='".$width."' class='img-circle' /></a>";
			else
	...
	public static function gridDisplay($val , $field, $arr) {
		...
		{
			if($arr['display']=='first_name')
      $arr['display']="first_name|last_name";
			...
			if(count($Q) >= 1 )
			{
				...
				if($arr['display']=='first_name|last_name')
         return $v="<p style='color:red;'>User not found !</p>";
				else
				return '';
			}
		} else {
	...
	{
		$avatar = '<img alt="" src="//www.gravatar.com/avatar/'.md5(Session::get('email')).'" class="img-circle user-profile-pic" width="'.$width.'" />';
		...
		if(count($Q)>=1)
		{
			...
			if($row->avatar !='' )
			{
				...
				{
					return  '<img src="'.asset('uploads/users').'/'.$row->avatar.'" border="0" width="'.$width.'" class="img-circle user-profile-pic" />';
				} else {
					return $avatar;
				}
			} else {
  ...
  static function CmsSettings(){
	 $get = CmsSettings::first();

	 if($get){

		 	if(file_exists('uploads/cms/logo/'.$get->logo_picture) && $get->logo_picture!='')
				$logo = '/uploads/cms/logo/'.$get->logo_picture;
		 	else
			 $logo = '';
			if(file_exists('uploads/cms/logo/'.$get->login_page_logo) && $get->login_page_logo!='')
			 $login_page_logo = '/uploads/cms/logo/'.$get->login_page_logo;
		 	else
			  $login_page_logo = '';
			 if(file_exists('uploads/images/'.$get->l_p_b_image) && $get->l_p_b_image!='')
				$l_p_b_image = '/uploads/images/'.$get->l_p_b_image;
		 	else
			 $l_p_b_image = '';
			 if(file_exists('uploads/images/'.$get->f_p_b_image) && $get->f_p_b_image!='')
				$f_p_b_image = '/uploads/images/'.$get->f_p_b_image;
		 	else
			 $f_p_b_image = '';
		...
		return array(
							'logo' =>$logo,
							'logo_text' =>$get->logo_text,
							'l_p_b_image'=>$l_p_b_image,
							'f_p_b_image'=>$f_p_b_image,
							'login_page_bg_size'=>($get->login_page_bg_size)?$get->login_page_bg_size:'',
							'login_page_color'=>($get->login_page_color)?$get->login_page_color:'',
							'l_p_w_note'=>($get->l_p_w_note)?$get->l_p_w_note:'',
							);
		}
	 ...
	 public static function activities($data){
		$activ=new Activities;
	    $activ->user_id=Auth()->user()->id;
	    $activ->from_user=Auth()->user()->first_name."&nbsp;".Auth()->user()->last_name;
	    $activ->message=$data['message'];
	    if(isset($data['picture']))
	    $activ->picture=$data['picture'];
	     if(isset($data['status']))
	    $activ->status=$data['status'];
	    $activ->save();

		 }

	public static function formActivities($data){
		$activities=$data['model']->find($data['data']['id']);
		if(count($activities)>0){
			 foreach($activities['original'] as $key=>$value){

				   if(array_key_exists($key,$data['data']) && $key!=="logo" && $key!=="avatar"){

						if($activities->$key!=$data['data'][$key]){
							 $data['message']=$data['update_create']."&nbsp;".$key;
							 if($value!='' && $data['data'][$key]!='')
								$data['status']= $value." To ".$data['data'][$key];
							elseif($value=='')
								$data['status']= $data['data'][$key];
							elseif($data['data'][$key]=='')
							    $data['status']= "Null";
						   ...
						   if($key=="logo"){
							    $pic['picture']='/uploads/logo/'.$value;
							    $pic['message']=$data['update_create']."&nbsp;".$key;
							     self::activities($pic);
  ...
  public static function cal_time($data){
	$time=strtotime($data);
    $time = time() - $time;
    $time = ($time<1)? 1 : $time;
    $tokens = array (
...
public static function getUsersSchool($userId,$groupId)
{
			$user = User::find($userId);
			if(!$user) return;

			if($groupId == 2){
					$getSchools = \DB::table('support_agent_schools')->where('user_id',$userId)->lists('school_id');
			}
			elseif($groupId == 4){
					$getSchools = \DB::table('regional_admin_schools')->where('user_id',$userId)->lists('school_id');
			}
			elseif($groupId == 9){
					$getSchools = \DB::table('coach_schools')->where(['user_id' => $userId, 'assign_type' => 'school'])->lists('school_id');
			}
			else{
					$getSchools = [$user->school_id];
			}


	if($getSchools && count($getSchools)>0 )
		$getSchools = School::whereIn('id',$getSchools)->lists('name')->toArray();
		return implode(', ',$getSchools);

		return;
}


	public static function getRegionalAdminSchoolsIds($userid){
		$getSchools = schoolassignregionaladmin::where('user_id',$userid)->lists('school_id')->toArray();
		if($getSchools && count($getSchools)>0 ){
				 $getSchool = School::whereIn('id',$getSchools)->lists('id')->toArray();
				 $getSchool = implode(',',$getSchool);
				 return ($getSchool)? $getSchool : 0;
			 }

		}

		public static function getRegionalAdminSchoolsNames($userid){
			$getSchools = schoolassignregionaladmin::where('user_id',$userid)->lists('school_id')->toArray();
			if($getSchools && count($getSchools)>0 ){
					 $getSchool = School::whereIn('id',$getSchools)->lists('name','id')->toArray();
					 return ($getSchool)? $getSchool :array();
				 }

			}

			public static function getSupportAdminSchoolsNames($userid){
				$getSchools = Schoolassign::where('user_id',$userid)->lists('school_id')->toArray();
				if($getSchools && count($getSchools)>0 ){
						 $getSchool = School::whereIn('id',$getSchools)->orWhere('entry_by',$userid)->lists('name','id')->toArray();
						 return ($getSchool)? $getSchool :array();
					 }
			}


		public static function getAssignedSchoolNames($surveyID)
		{
				$schoolName = School::join('survey_assign', 'schools.id', '=', 'survey_assign.school_id')->where('survey_assign.survey_id', $surveyID)->lists('schools.name');
				if($schoolName)
				 	$schoolName = implode(', ',json_decode($schoolName,true));
				else
					$schoolName = '';
		...
		public static function getSurveyassign($id){
			$schoolIds = Surveyassign::where('survey_assign.survey_id',$id);

			if(Auth::user()->group_id == 9){
				$schoolIds = $schoolIds->join('coach_schools', 'coach_schools.school_id', '=', 'survey_assign.school_id');
			}
			elseif(Auth::user()->group_id == 2){
				$schoolIds = $schoolIds->join('support_agent_schools', 'support_agent_schools.school_id', '=', 'survey_assign.school_id');
			}
			elseif(Auth::user()->group_id == 4){
				$schoolIds = $schoolIds->join('regional_admin_schools', 'regional_admin_schools.school_id', '=', 'survey_assign.school_id');
			}

			$schoolIds = $schoolIds->lists('survey_assign.school_id')->toArray();
			return  $schoolIds;
		}

		public static function getAllSchoolIds(){
			return School::lists('name','id')->toArray();
		}


	public static function getSchool($id){
			return School::find($id)->first();
	}


	public static function getEmailTemplatesSubject(){
		 $subjects=self::getEmailTemplatesSubjectAndFrom();
     return $subjects;
	}

	 public static function getEmailTemplatesSubjectAndFrom(){
	 $s_f=Settings::lists('value','name');
	 if($s_f){
		foreach($s_f as $key => $value){
	...
	public static function getFileUpload($file,$destinationPath=null,$name=null)
	{
		if($file)
		{
			...
			$newfilename =($name!='')?$name.'.'.$extension:strtotime(date('Y-m-d H:i:s')).'-'.$rand.'.'.$extension;
			...
			if(file_exists($destinationPath.'/'.$newfilename))
			 return ['error'=>'File Name Allready Exiest !'];
	...
	public static function getMultipleReportsSurvey($id){
			$survey = Survey::find($id);
			$surveyIDS = explode(',', $survey->multiple_report_survey_ids);
			$surveyLists = Survey::whereIn('id', $surveyIDS)->lists('name','unique_key');
			return (count($surveyLists))?json_decode($surveyLists,true):[];

	}


	public static function getUnitAssignedSections($unitId){
	return	Lmssections::select('lms_sections.id','lms_sections.title')
					->join('lms_assign','lms_sections.id','=','lms_assign.lms_section_id')
					->where('lms_assign.lms_unit_id',$unitId)
					->lists('lms_sections.title','lms_sections.id');

	}


	public static function getUnitAssignedModules($unitId){

		return Lmsmodules::select('lms_modules.title','lms_modules.id')
						->join('lms_assign','lms_modules.id','=','lms_assign.lms_module_id')
						->where('lms_assign.lms_unit_id',$unitId)
						->lists('lms_modules.title','lms_modules.id');
	}


	public static function getSectionAssignedModules($sectionId){

		return Lmsmodules::select('lms_modules.title','lms_modules.id')
						->join('lms_assign','lms_modules.id','=','lms_assign.lms_module_id')
						->where('lms_assign.lms_section_id',$sectionId)
						->where('lms_assign.lms_unit_id','0')
						->lists('lms_modules.title','lms_modules.id');
	}

	public static function createdTime($created_at,$tokens){
	...
	public static function geterrorcontent(){
     $data = Settings::where('name','errorpage')->first();
		 $data = (isset($data->value) &&  strip_tags(json_decode($data->value))!='')?json_decode($data->value):'';

		 return $data;
	}


	public static function getLMSCompletedStatus($moduleId, $sectionId, $unitId)
	{
		return \DB::table('lms_analytics')->select('unit_completed_status')
								->where(['user_id' => \Auth::id(), 'module_id' => $moduleId, 'section_id' => $sectionId, 'unit_id' => $unitId, 'unit_completed_status' => 1])
								->first();
	}
	...
	public static function getSurveyAssignedSections($surveyId){
	return	Lmssections::join('lms_survey_assign','lms_sections.id','=','lms_survey_assign.section_id')
					->where('lms_survey_assign.survey_id',$surveyId)
					->select('lms_survey_assign.id', 'lms_sections.title')
					->lists('lms_sections.title','lms_survey_assign.id');

	}


	public static function getSurveyAssignedModules($surveyId){

		return	Lmsmodules::join('lms_survey_assign','lms_modules.id','=','lms_survey_assign.module_id')
						->where('lms_survey_assign.survey_id',$surveyId)
						->select('lms_survey_assign.id', 'lms_modules.title')
						->lists('lms_modules.title','lms_survey_assign.id');
	}


	public static function getSurveyAssignedUnits($surveyId){

		return	Lmsunits::join('lms_survey_assign','lms_units.id','=','lms_survey_assign.unit_id')
						->where('lms_survey_assign.survey_id',$surveyId)
						->select('lms_survey_assign.id', 'lms_units.title')
						->lists('lms_units.title','lms_survey_assign.id');
	}




	public static function getLMSSurvey($moduleId,$sectionId,$unitId)
	{

			return Lmssurveyassign::join('survey', 'lms_survey_assign.survey_id', '=', 'survey.id')
															->where('module_id', $moduleId)
															->orWhere('section_id', $sectionId)
															->orWhereIn('unit_id', $unitId)
															->select('survey.*', 'lms_survey_assign.id as assign_id')
															->groupBy('survey.id')
															->orderBy('lms_survey_assign.order_by')
															->get();
	}

	public static function lmsmodulesCheckBuyed($module_id){
					return Lmsorders::where(['user_id'=>(Auth::id())?Auth::id():0,'lms_module_id'=>$module_id])
															->first();

	}

	public static function getDiscussionsReply($discussions_id){
      return  Lmsdiscussionreplies :: select('lms_discussion_replies.*','tb_users.first_name','tb_users.last_name')
			        ->join('tb_users','tb_users.id','=','lms_discussion_replies.user_id')
			        ->where('lms_discussion_id',$discussions_id)
							->orderBy('id','decs')
							->get();
	}


	public static function getLMSSurveyStatus($surveyId,$userId)
	{
			$check = Answer::where(['survey_id' =>$surveyId, 'user_id' => $userId])->first();
			return ($check)?$check->created_at:0;
	}

	public static function getStripeDetail(){
		$getStripeDetail=Settings::where('name','stripe')->first();
		return isset($getStripeDetail->value)?json_decode($getStripeDetail->value):(object)array('stripe_publishable'=>'','stripe_secret'=>'');
	}


	public static function checkLMSPermission($module=null){
			if(!Auth::id())
					return 0;

			$user = Auth::user();
			$finalPermission = ['lmsmodules' =>[], 'lmssections' =>[], 'lmsunits' =>[], 'lmsresource' =>[], 'lmssurvey' =>[], 'lmscategories' =>[]];
			$blank = ['view' => '', 'add' => '', 'edit' => '', 'delete' => ''];

			$permissions = new Lmspermission;

			if($user->group_id == '3' || $user->group_id == '4')
			$permissions = $permissions->where('schools_assigned', 'LIKE', '%'.$user->school_id.'%');

			$permissions = $permissions->orWhere('groups_assigned', 'LIKE', '%'.$user->group_id.'%');
			$permissions = $permissions->orWhere('users_assigned', 'LIKE', '%'.$user->id.'%');
			$permissions = $permissions->lists('permission', 'id');
			...
			foreach ($permissions as $permission) {
			  ...
			  foreach($permission as $key => $val){
						if(isset($finalPermission[$key]))
			      $finalPermission[$key] = $finalPermission[$key]+$val;
			  }

				if(is_array($permission)){
						foreach ($permission as $key => $value) {
							if(isset($finalPermission[$key]))
							$finalPermission[$key] = $finalPermission[$key]+$blank;
						}
				}else{
					foreach ($finalPermission as $key => $value) {
						if(isset($finalPermission[$key]))
						$finalPermission[$key] = $finalPermission[$key]+$blank;
					}
				}
			...
			if($module && isset($finalPermission[$module])){
					if(count($finalPermission[$module]))
							return $finalPermission[$module];
					else
							return $finalPermission[$module]+$blank;
			}
			else
...
public static function getSettingsValue($name){
		$getSettings = Settings::where('name',$name)->first();
		return ($getSettings)? $getSettings->value: '';
}
...
{
   Notifications::save_notifications($request);
}


public static function getProfileCompletedStatus()
{
		$totalField = 5;
		$unfilled = [];
		$status = 100;

		$user = User::select('tb_users.first_name', 'tb_users.email', 'tb_users.phone', 'tb_users.profile_title', 'tb_users.about')->find(\Auth::id());
		...
		$unfilled = $totalField - count($unfilled);

		if($unfilled != $totalField){
			$status = ($unfilled * 100) / $totalField;
		}
