...
while(! feof($file))
  {
  $getLine = fgets($file);
  if (strpos($getLine,'DB_HOST') !== false) {
		$DB_HOST = trim(str_replace('DB_HOST=','',$getLine));
	}
  if (strpos($getLine,'DB_DATABASE') !== false) {
		$DB_DATABASE = trim(str_replace('DB_DATABASE=','',$getLine));
	}
  if (strpos($getLine,'DB_USERNAME') !== false) {
		$DB_USERNAME = trim(str_replace('DB_USERNAME=','',$getLine));
	}
  if (strpos($getLine,'DB_PASSWORD') !== false) {
		$DB_PASSWORD = trim(str_replace('DB_PASSWORD=','',$getLine));
	}
  }
...
*/
 ...
 $ACCESS_PWD='sonu@admin'; #!!!IMPORTANT!!! this is script access password, SET IT if you want to protect you DB from public access

 #DEFAULT db connection settings
 # --- WARNING! --- if you set defaults - it's recommended to set $ACCESS_PWD to protect your db!
 $DBDEF=array(
 'user'=>$DB_USERNAME,#required
